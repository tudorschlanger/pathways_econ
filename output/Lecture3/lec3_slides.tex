...
\begin{frame}{How Economists Think}
  ...
  \begin{tikzpicture}[
    ...
    \node[nd, fill=vermillion!15, draw=vermillion] (obs) at (0,4)
      {\textbf{Observe the problem}\\[-1pt]
       {\scriptsize e.g., Policy 101 depleted all candy}};
    ...
    \node[nd, fill=green!12, draw=green] (design) at (0,0.1)
      {\textbf{Design better rules}\\[-1pt]
       {\scriptsize e.g., Policy 102 puts a price on overuse}};
...
% ---------- Policy Comparison Diagram ----------
\begin{frame}{The Problem}

  \begin{center}
  \begin{tikzpicture}[
    pol/.style={rounded corners=4pt, minimum width=3.2cm,
                minimum height=0.9cm, align=center, font=\small\bfseries},
    stp/.style={rounded corners=2pt, draw=DarkGray, fill=softbg,
                minimum width=3.2cm, minimum height=0.7cm,
                align=center, font=\scriptsize},
    res/.style={rounded corners=2pt, minimum width=3.2cm,
                minimum height=0.7cm, align=center, font=\scriptsize\bfseries},
    ar/.style={-{Stealth[length=4pt]}, thick, DarkGray}
  ]
    % --- Policy 101 (left) ---
    \node[pol, fill=vermillion!20, draw=vermillion] (p1) at (-3.5,3)
      {Policy 101\\{\scriptsize \$0 price}};
    \node[stp] (p1s1) at (-3.5,1.8) {Everyone grabs\\as much as they want};
    \node[font=\scriptsize\bfseries, text=blue, anchor=west]
      at ([xshift=4pt]p1s1.east) {$\leftarrow$ Demand};

    \node[stp] (p1s2) at (-3.5,0.6) {Bowl empties\\in seconds};
    \node[font=\scriptsize\bfseries, text=orange, anchor=west]
      at ([xshift=4pt]p1s2.east) {$\leftarrow$ Supply};
    \node[res, text=vermillion] (p1out) at (-3.5,-0.7)
      {Depletion +\\Unfair distribution};
    ...
    % --- Policy 102 (right) ---
    % \node[pol, fill=green!15, draw=green] (p2) at (3.5,3)
    %   {Policy 102\\{\scriptsize \$5 per extra piece}};
    % \node[stp] (p2s1) at (3.5,1.8) {Students compare cost\\to willingness to pay};
    % \node[stp] (p2s2) at (3.5,0.6) {People take less\\candy remains};
    % \node[res, text=green] (p2out) at (3.5,-0.7)
    %   {Conservation +\\Fairer outcome};
    ...
    % % --- Center comparison ---
    % \node[font=\Large\bfseries, text=DarkGray] at (0,3) {vs.};
    % \draw[dotted, thick, DarkGray] (0,2.5) -- (0,-1.1);

    % % --- Bottom insight ---
    % \node[text width=10cm, align=center, font=\small] at (0,-1.7)
    %   {The \textbf{same people}, the \textbf{same candy} ---
    %    only the \textbf{rules} changed.};
  \end{tikzpicture}
  \end{center}
...
\begin{frame}{Rivalrous goods}
  ...
  \begin{tikzpicture}[
    ...
    lbl/.style={font=\small\bfseries, text=DarkGray}
  ]
    % Column headers
    ...
    \node[cell] at (2.65,2) {Store clothing\\Cars at a dealership};
    \node[cell] at (7.35,2) {Netflix movies\\Toll road};
    \node[cell] at (2.65,0) {Ocean fish\\Groundwater};
    \node[cell] at (7.35,0) {National defense\\Public parks};
  \end{tikzpicture}
  \end{center} \pause 

  \begin{questionbox}
    What is the difference between top and bottom rows?
  \end{questionbox}
...
\begin{frame}{Excludable Goods}
  ...
  \begin{tikzpicture}[
    ...
    \node[cell] at (2.65,2) {Store clothing\\Cars at a dealership};
    \node[cell] at (7.35,2) {Netflix movies\\Toll road};
    \node[cell] at (2.65,0) {Ocean fish\\Groundwater\\[2pt]
...
\begin{frame}{Tragedy of the Commons}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}
        \item The good is \textbf{non-excludable} and no price to ration it
        \item The demand for the good is infinite 
        \item The good is \textbf{rivalrous} because its supply is finite 
  ...
  \pause 
      \begin{questionbox}
        Are people ``bad''? Why can't they just self-regulate their behavior? 
      \end{questionbox}
...
\begin{frame}{Negative Externalities}
  ...
  \begin{definitionbox}[Externality]
    An \textbf{externality} is a \textbf{cost} or benefit that affects someone \textbf{who is not directly involved} in a decision or transaction. 
  \end{definitionbox}

  \vspace{0.5em}
  \textbf{Candy example}
  \begin{itemize}
    \item \textit{Today}, I collect a lot of candy. 
    \item This affects the people who want candy \textit{tomorrow}. 
    \item The cost is the lack of candy. 
  \end{itemize} \pause 
...
\begin{frame}{How Can We Fix the ``Tragedy of the Commons''?}
  ...
  \begin{insightbox}
    ...
    This is the logic behind taxes, fines, and cap-and-trade systems.
  \end{insightbox}
...
\begin{frame}{A Policy Proposal: Tax Carbon Emissions}
  ...
  \begin{definitionbox}[Carbon Tax]
    A fixed price charged for every \textbf{tonne of CO\textsubscript{2}} emitted.
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{How it works:}
      \begin{itemize}
        \item Government sets a price, e.g.\ \textbf{\$50 per tonne} of CO\textsubscript{2}
        \item Firms and households pay when they emit
        \item Higher price $\rightarrow$ stronger incentive to emit less
      \end{itemize}
    \end{column}
    \pause
    \begin{column}{0.48\textwidth}
      \textbf{Why economists like it:}
      \begin{itemize}
        \item Partially corrects the \textbf{negative externality}
        \item Lets people \textbf{choose how} to reduce 
        \item Generates \textbf{revenue} the government can spend or return
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{What Does \$50 per Tonne of CO\textsubscript{2} Mean?}

  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \textbf{Gas price example:}
      \begin{itemize}
        \item 1 gallon of gasoline produces \textbf{8.9 kg} of CO\textsubscript{2}
        \item Carbon tax = \$50 per tonne = \$0.05 per kg
        \item Extra cost per gallon: $8.9 \times \$0.05 = $ \textbf{\$0.44}
      \end{itemize}

      \vspace{0.5em}
      \begin{answerbox}
        \centering
        Gas price: \$3.50 $\rightarrow$ \textbf{\$3.94 per gallon}\\[0.3em]
    ...
    \begin{column}{0.42\textwidth}
      \begin{insightbox}
        ...
        \vspace{0.3em}
        \begin{itemize}
          \item Firms and households
          \item Whether you are rich or poor
        \end{itemize}
        ...
        \vspace{0.3em}
        \begin{itemize}
          \item Poor more than the rich
          \item People who drive more to work 
        \end{itemize}
      \end{insightbox}
    \end{column}
  \end{columns}
...
\begin{frame}{What Should the Government Do with the Revenue?}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{How much revenue?}
      \begin{itemize}
        \item U.S.\ emits $\approx$ 5 billion tonnes of CO\textsubscript{2}/year
        \item At \$50/tonne: $5\text{B} \times \$50 = $ \textbf{\$250 billion/year}
        \item That is roughly \textbf{\$1{,}900 per household}
              {\scriptsize (130 million U.S.\ households)}
      \end{itemize}
    \end{column}
    \pause
    \begin{column}{0.48\textwidth}
      \textbf{Options for the revenue:}
      \begin{enumerate}
        \item \textbf{\textcolor{orange}{Return it to households}} as a rebate check
        \item \textbf{\textcolor{skyblue}{Invest}} in clean energy or transit
        \item \textbf{\textcolor{green}{Cut other taxes}} (e.g.\ income tax)
      \end{enumerate}
    \end{column}
  \end{columns}
  ...
  \pause
  \vspace{0.5em}
  \begin{definitionbox}[Canada's Real-World Example]
    Canada introduced a carbon tax of CA\$80/tonne in 2024.
    \textbf{90\% of revenue is returned directly to households} via quarterly rebate checks (the Climate Action Incentive Payment).

    \vspace{0.3em}
    Result: \textbf{8 out of 10 households get back more than they pay} in higher prices.

    \vspace{0.2em}
    {\scriptsize Source: Canada Parliamentary Budget Officer (2024).}
  \end{definitionbox}
...
\begin{frame}{Stakeholder Analysis: Government}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{\textcolor{green!60!black}{Benefits}}
      \begin{itemize}\small
        \item New revenue stream (\$250B/year at \$50/tonne)
